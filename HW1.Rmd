...
```{r distinct}
dur_word_freq %>% select("Lemma") %>%  distinct()
...
```{r is_na}
dur_word_freq %>% map_int(~any(is.na(.x)))
```
...
```{r mutate}
dur_word_freq %>% mutate(across(where(is.character),as.factor)) -> dur_word_freq
dur_word_freq %>% str()
```
...
```{r summarise}
dur_word_freq %>% group_by(Cond) %>% summarise(meanLogDurA = mean(LogDurationA),
...
dur_word_freq %>% group_by(Cond) %>% summarise(meanLogDurA = mean(LogDurationA),
                                               ...
                                               medLogDurA = median(LogDurationA),
                                               medLogDurW = median(LogDurationW),)
```
...
```
При быстром произнесении длительность гласных ниже, что по медиане, что по квартилям, при этом разница между первыми квартилями выше, чем между медианами, а между медианами - выше, чем между третьими квартилями
```
...
dur_word_freq %>%
  ...
  geom_boxplot() +
  theme_classic() +
  xlab("Manner of pronounciation") + ylab("Log-scaled word length")
```
...
dur_word_freq %>%
  ...
  geom_boxplot() +
  theme_classic() +
  xlab("Manner of pronounciation") + ylab("Log-scaled segment length")
```
...
dur_word_freq %>%
  ...
  xlab("Log-scaled segment length") + ylab("Log-scaled word length")
```
...
```
Длина слова положительно коррелирует с длиной сегмента
```
...
```{r wider}
dur_word_freq %>% filter(Cond == "slow" & Exponent == "t") %>% select("Participant", "Lemma", "LogDurationW") %>% 
  reshape(idvar = "Participant", timevar = "Lemma", direction = "wide")
```
...
```{r verb}
dur_word_freq %>% filter(Cond == "slow" & Exponent == "t") %>% select("Participant", "Lemma", "LogDurationW") %>% 
  reshape(idvar = "Participant", timevar = "Lemma", direction = "wide") %>% apply(2, function(x) sum(is.na(x)))
```
